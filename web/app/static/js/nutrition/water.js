...
function updateWaterVisual() {
    if (!waterWidget || !waterCurrent || !waterProgress) {
        return;
    }
    ...
    if (recommendedWater <= 0) {
        ...
        waterCurrent.textContent = currentWater.toFixed(1);
        waterProgress.textContent = "Рекомендація недоступна";
    ...
    waterCurrent.textContent = currentWater.toFixed(1);
    waterProgress.textContent = `${Math.round(ratio * 100)}% від рекомендованого`;
}

function applyWaterData(data) {
    currentWater = Number(data.amount ?? 0);
    recommendedWater = Number(data.recommended ?? 0);
    updateWaterVisual();
}
