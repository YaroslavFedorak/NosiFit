...
function renderSleepIcon() {
    const icon = getElement("#sleep-header-icon");
    if (!icon) {
        return;
    }
    icon.innerHTML =
...
async function saveSleep() {
    ...
    try {
        await RecoveryAPI.addSleep(userId, startDate.toISOString(), endDate.toISOString());
...
function bindModal(backdrop, openButton, closeButton, saveButton) {
...
export function initSleepModal() {
    ...
    const initialize = () => {
        ...
        renderSleepIcon();
        initialized = true;
        bindModal(backdrop, openButton, closeButton, saveButton);
    };
    initialize();
    if (!initialized) {
        requestAnimationFrame(() => {
