...
function createCalendar(year, month, dayMap, onMonthChange) {
    ...
    for (let dayNumber = 1; dayNumber <= daysInMonth; dayNumber += 1) {
        ...
        grid.appendChild(createCalendarDay(day, month, year));
    }
...
export function openCalendarModal(data) {
    const modal = document.getElementById("dashboard-calendar-modal");
    const content = document.getElementById("dashboard-calendar-content");
    if (!modal ||
        !content) {
        return;
    }
    ...
    const render = () => {
        content.innerHTML =
            "";
        const calendar = createCalendar(currentYear, currentMonth, dayMap, (year, month) => {
...
export function initCalendarModal(data) {
    const modal = document.getElementById("dashboard-calendar-modal");
    if (!modal) {
        return;
    }
    ...
    const openButton = document.getElementById("dashboard-open-calendar");
