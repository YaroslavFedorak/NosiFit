...
function createInput(exercise, field, value, label) {
    ...
    const arrows = el("div", {
        ...
        el("div", {
            class: "db-arrow db-arrow-up"
        }),
        el("div", {
            class: "db-arrow db-arrow-down"
        })
    ]);
    const arrowUp = arrows.children[0];
    const arrowDown = arrows.children[1];
    arrowUp.setAttribute("data-exercise-id", exercise.id);
    arrowUp.setAttribute("data-field", field);
    arrowUp.setAttribute("data-action", "arrow-up");
    arrowDown.setAttribute("data-exercise-id", exercise.id);
    arrowDown.setAttribute("data-field", field);
    arrowDown.setAttribute("data-action", "arrow-down");
    return el("div", {
        class: "db-input-inline"
    }, [
        input,
        el("span", {
            class: "db-input-inline-label",
            text: label
        }),
        arrows
    ]);
...
function createCheckbox(exercise) {
    const button = el("button", {
        class: `db-ex-check ${exercise.completed
            ? "checked"
            : ""}`,
        type: "button",
...
function createExerciseRow(exercise) {
    const row = el("div", {
        class: `db-session-ex-row ${exercise.completed
            ? "db-ex-done"
            : ""}`
    });
    row.setAttribute("data-exercise-id", exercise.id);
    row.appendChild(el("div", {
        class: "db-session-ex-name-wrap"
    }, [
        el("div", {
...
export function render(container) {
    ...
    const active = exercises.filter(exercise => !exercise.completed);
    const completed = exercises.filter(exercise => exercise.completed);
    const fragment = document.createDocumentFragment();
    active.forEach(exercise => {
        fragment.appendChild(createExerciseRow(exercise));
    });
