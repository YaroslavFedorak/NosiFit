...
function normalizeHabits(snapshot) {
    ...
    const rawHabits = snapshot.habits;
    ...
    return rawHabits
        ...
        .map(value => {
        const raw = value;
        const userHabit = raw.user_habit &&
            typeof raw.user_habit ===
                "object"
            ? raw.user_habit
            : null;
        const habit = raw.habit &&
            typeof raw.habit ===
                "object"
            ? raw.habit
            : null;
        const userHabitId = raw.user_habit_id ??
            raw.userHabitId ??
            userHabit?.id;
        const id = raw.id ??
            raw.habit_id ??
        ...
        .filter(habit => Number(habit.user_habit_id) > 0);
}
function createHabitItem(habit) {
    const item = document.createElement("div");
    item.className =
        `habit-item ${habit.completed
            ? "habit-completed"
            : ""} habit-user`;
    if (habit.category) {
    ...
    check.addEventListener("click", async () => {
        ...
        try {
            ...
            console.error("Failed to log recovery habit", error);
            if (wasCompleted) {
    ...
    removeButton.addEventListener("click", async () => {
        const userHabitId = removeButton.dataset
            .userHabitId;
        if (!userHabitId) {
            return;
        }
        if (!confirmationPending) {
            confirmationPending =
                true;
            removeButton.classList.add("habit-remove-pending");
            confirmationTimeout =
                setTimeout(() => {
                    confirmationPending =
                        false;
                    removeButton.classList.remove("habit-remove-pending");
                    confirmationTimeout =
                        null;
                }, 2000);
            return;
        ...
        try {
            ...
            console.error("Failed to remove recovery habit", error);
        }
        finally {
            confirmationPending =
                false;
...
function createHabitsList(habits) {
    ...
    habits.forEach((habit, index) => {
        const column = index % 2 === 0
            ? left
            : right;
        column.appendChild(createHabitItem(habit));
    });
    grid.appendChild(left);
    grid.appendChild(right);
    return grid;
}
function createContainer(habits) {
    ...
    container.appendChild(createHabitsList(habits));
    return container;
}
export function renderHabitsWidget(snapshot) {
    const element = document.getElementById("dashboard-habits-list");
    if (!element) {
        return;
    }
    element.innerHTML =
    ...
    element.appendChild(createContainer(preview));
}
