import type { RecoveryHabit, RecoverySnapshot } from "./state";
...
function getReason(category?: string): string {
    if (!category) {
...
function getIcon(habit: RecoveryHabit): string {
...
function createHabitItem(habit: RecoveryHabit): HTMLElement {
...
export function renderHabits(snapshot: RecoverySnapshot | null): void {
    const countEl = getElement<HTMLElement>("dashboard-habits");
    const listEl = getElement<HTMLElement>("dashboard-habits-list");
...
export function bindHabitActions(
    ...
    listEl.addEventListener("click", async (event) => {
        ...
        const check = target.closest<HTMLButtonElement>(".habit-check");
        ...
        try {
            await fetch(
                `/api/recovery/habits/${encodeURIComponent(userHabitId)}`,
                {
                    method: "DELETE"
                }
            );
