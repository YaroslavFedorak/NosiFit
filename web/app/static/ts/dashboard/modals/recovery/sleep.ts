...
function getTomorrowDate(date: string): string {
...
function closeModal(): void {
    const backdrop = getElement<HTMLElement>(
        "#sleep-modal-backdrop"
    );

    const dateInput = getElement<HTMLInputElement>(
        "[data-sleep-date]"
    );

    const startInput = getElement<HTMLInputElement>(
        "[data-sleep-start-time]"
    );

    const endInput = getElement<HTMLInputElement>(
        "[data-sleep-end-time]"
    );
...
async function saveSleep(): Promise<void> {
    ...
    try {
        ...
        window.dispatchEvent(
            new CustomEvent("dashboard:recovery-updated")
        );
    } catch (error) {
        console.error(
...
export function initSleepModal(): void {
    ...
    const openButton = getElement<HTMLButtonElement>(
        "#dashboard-add-sleep"
    );
